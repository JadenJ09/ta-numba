...
class WilliamsRStreaming(StreamingIndicator):
    ...
    def update(self, high: float, low: float, close: float) -> float:
        ...
        if len(self.high_buffer) >= self.window:
            ...
            if highest_high != lowest_low:
                self._current_value = -100 * (highest_high - close) / (highest_high - lowest_low)
            else:
                self._current_value = -100.0
...
class TSIStreaming(StreamingIndicator):
    ...
    def update(self, value: float) -> float:
        ...
        # Calculate TSI
        if not np.isnan(smooth2_momentum) and not np.isnan(smooth2_abs) and smooth2_abs != 0:
            self._current_value = 100 * (smooth2_momentum / smooth2_abs)
            
            # TSI is ready when both double smoothed values are ready
            if (self.momentum_ema1.is_ready and self.momentum_ema2.is_ready and 
                self.abs_momentum_ema1.is_ready and self.abs_momentum_ema2.is_ready):
                self._is_ready = True
...
from .trend import SMAStreaming, EMAStreaming